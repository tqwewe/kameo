...
async fn test_no_duplicate_local_registration() {
    ...
    let found = ActorRef::<TestActor>::lookup("duplicate_test").await.unwrap();
    assert!(found.is_some());
...
async fn test_multiple_actor_registration() {
    ...
    let found1 = ActorRef::<TestActor>::lookup("actor_1").await.unwrap().unwrap();
    let found2 = ActorRef::<TestActor>::lookup("actor_2").await.unwrap().unwrap();
    let found3 = ActorRef::<TestActor>::lookup("actor_3").await.unwrap().unwrap();
...
async fn test_lookup_nonexistent_actor() {
    let result = ActorRef::<TestActor>::lookup("nonexistent_actor").await.unwrap();
