...
2. **Message Serialization with `distributed_actor!`**: In Kameo, messages sent between nodes must be serializable. Message types need to implement the required serialization traits, and the actor must be registered with the `distributed_actor!` macro.
