...
where
    M: HasTypeHash + Send + Archive + for<'a> RSerialize<
    ...
    >,
{
    ...
    pub async fn send_pre_serialized(&mut self, type_hash: u32, data: bytes::Bytes) -> Result<(), StreamError> {
        self.write_frame_owned(type_hash, data).await
    }
    
    /// Send pre-serialized rkyv data directly (zero-copy from AlignedVec)
    pub async fn send_rkyv_aligned(&mut self, type_hash: u32, aligned_vec: rkyv::util::AlignedVec) -> Result<(), StreamError> {
    ...
    pub async fn close(mut self) -> Result<(), StreamError> {
        ...
        self.connection.send_bytes_zero_copy(frame.freeze())
    ...
    async fn write_frame_owned(&mut self, type_hash: u32, data: bytes::Bytes) -> Result<(), StreamError> {
    ...
    async fn write_single_frame(&mut self, type_hash: u32, data: &[u8]) -> Result<(), StreamError> {
        // Create StreamHeader
        let header = kameo_remote::StreamHeader {
            stream_id: self.stream_id,
        ...
        let header_bytes = header.to_bytes();
        let frame_size = 8 + header_bytes.len() + data.len(); // Protocol header + StreamHeader + data
        
        let mut frame = BytesMut::with_capacity(4 + frame_size);
        frame.put_u32(frame_size as u32);
        frame.put_u8(0x11); // MessageType::StreamData
        frame.put_u16(0); // Correlation ID (none for streaming)
        frame.put_slice(&[0u8; 5]); // Reserved
        frame.put_slice(&header_bytes);
    ...
    /// Write chunked frames for large messages
    async fn write_chunked_frames(&mut self, type_hash: u32, data: &[u8]) -> Result<(), StreamError> {
        ...
        self.connection.send_bytes_zero_copy(start_frame.freeze())
            .map_err(|_| StreamError::ConnectionClosed)?;
        
        // Send data chunks
        for (chunk_index, chunk) in data.chunks(MAX_CHUNK_SIZE).enumerate() {
            ...
            let frame_size = 8 + header_bytes.len() + chunk.len();
            
            let mut frame = BytesMut::with_capacity(4 + frame_size);
            frame.put_u32(frame_size as u32);
            frame.put_u8(0x11); // MessageType::StreamData
            frame.put_u16(0); // Correlation ID
            frame.put_slice(&[0u8; 5]); // Reserved
            frame.put_slice(&header_bytes);
        ...
        self.connection.send_bytes_zero_copy(end_frame.freeze())
            .map_err(|_| StreamError::ConnectionClosed)?;
    ...
    async fn write_single_frame_owned(&mut self, type_hash: u32, data: bytes::Bytes) -> Result<(), StreamError> {
        // Create StreamHeader
        let header = kameo_remote::StreamHeader {
            stream_id: self.stream_id,
            total_size: data.len() as u64,
        ...
        self.connection.write_bytes_vectored(frame_header.freeze(), data)
            .map_err(|_| StreamError::ConnectionClosed)?;
    ...
    async fn write_chunked_frames_vectored(&mut self, type_hash: u32, data: bytes::Bytes) -> Result<(), StreamError> {
        const MAX_CHUNK_SIZE: usize = 256 * 1024; // 256KB chunks (matching kameo_remote)
        ...
        for batch_start in (0..total_chunks).step_by(CHUNKS_PER_BATCH) {
            ...
            self.connection.write_owned_chunks(batch_frames)
                .map_err(|_| StreamError::ConnectionClosed)?;
        }
        ...
        >,
{
    ...
    pub fn process_frame(&mut self, data: &[u8]) -> Result<Option<M>, Box<dyn std::error::Error + Send + Sync>> {
