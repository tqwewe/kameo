...
pub struct RemoteMessageHandler {
    /// Mapper for backward compatibility with string-based IDs
    string_to_hash_mapper: Arc<RwLock<HashMap<(String, String), TypeHash>>>,
}

impl RemoteMessageHandler {
    pub fn new() -> Self {
        Self {
            string_to_hash_mapper: Arc::new(RwLock::new(HashMap::new())),
        }
    }
    
    /// Register a type hash mapping for generic actors
    pub fn register_type_hash(
        &self,
        type_hash: TypeHash,
        message_fns: RemoteMessageFns,
    ) {
        TYPE_HASH_REGISTRY
            .write()
            .unwrap()
    ...
    /// Register a string-to-hash mapping for backward compatibility
    pub fn register_string_mapping(
        &self,
        actor_id: String,
        message_id: String,
        type_hash: TypeHash,
    ) {
        self.string_to_hash_mapper
            .write()
            .unwrap()
            .insert((actor_id, message_id), type_hash);
    }
}
    ...
    fn handle_tell(
        ...
        if let Some(hash_str) = message_type.strip_prefix("hash:") {
            if let Ok(type_hash) = u32::from_str_radix(hash_str, 16) {
                return self.handle_tell_typed(actor_id, type_hash, Bytes::copy_from_slice(payload));
            }
        }
        
        let msg_type = message_type.to_string();
        Box::pin(async move {
            Err(format!("Unsupported message type format: {}", msg_type).into())
        })
    }
        ...
        if let Some(hash_str) = message_type.strip_prefix("hash:") {
            if let Ok(type_hash) = u32::from_str_radix(hash_str, 16) {
                ...
                return Box::pin(async move {
                    handler.handle_ask_typed(actor_id, type_hash, payload_bytes)
                        .await
                ...
                return (tell_fn)(actor_id, payload, mailbox_timeout)
                    ...
                    .map_err(|e| match e {
                        RemoteSendError::UnknownMessage { actor_remote_id, message_remote_id } => {
                            format!("Unknown message: {}:{}", actor_remote_id, message_remote_id).into()
                        },
