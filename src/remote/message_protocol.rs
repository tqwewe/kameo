...
impl MessageHeaderV2 {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, MessageHeaderError> {
        ...
        let msg_type = MessageType::try_from(bytes[0])
            .map_err(|_| MessageHeaderError::InvalidMessageType)?;
        let correlation_id = u16::from_be_bytes([bytes[1], bytes[2]]);
        let flags = MessageFlags::from_bits(bytes[3])
            .ok_or(MessageHeaderError::InvalidFlags)?;
...
impl StringToHashMapper {
    ...
    pub fn get_hash(&self, actor_id: &str, message_id: &str) -> Option<TypeHash> {
        self.mappings.get(&(actor_id.to_string(), message_id.to_string())).copied()
    }
