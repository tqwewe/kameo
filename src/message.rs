    ...
    R: Reply + ?Sized,
{
    ...
    /// Creates a forked context for an inner message handler.
    ///
    /// The forked context shares the same `actor_ref` but has no reply sender and
    /// `stop` reset to `false`. After the inner handler returns, call
    /// [`__internal_should_stop`](Context::__internal_should_stop) to propagate the
    /// stop flag back to the outer context.
    ///
    /// This is used by the code generated by `#[messages(messages = …, replies = …)]`.
    /// Do not call it directly.
    #[doc(hidden)]
    pub fn __internal_fork<R2>(&self) -> Context<A, R2>
    where
        R2: AggregatableReply + ?Sized,
    {
        Context {
            actor_ref: self.actor_ref.clone(),
            reply: None,
            // Internal handlers operate within the same message processing cycle.
            // They have no reason to be aware of the external handler's decision
            stop: false,
        }
    }

    /// Returns whether the inner context requested the actor to stop.
    ///
    /// Used together with [`__internal_fork`](Context::__internal_fork) in the code
    /// generated by `#[messages(messages = …, replies = …)]`. Do not call it directly.
    #[doc(hidden)]
    pub fn __internal_should_stop(&self) -> bool {
        self.stop
    }
}
...
#[diagnostic::on_unimplemented(
    message = "`{Self}` cannot be used as a reply type in `#[messages(enum)]`",
    label = "this reply type doesn't satisfy `Reply<Value = Self>`",
    note = "`DelegatedReply` and `ForwardedReply` are not compatible with enum dispatch",
    note = "to fix: remove `enum` from `#[messages(enum)]`, or change the return type to a plain value"
)]
pub trait AggregatableReply: Reply<Value = Self> {}
impl<T> AggregatableReply for T where T: Reply<Value = T> {}
