...
impl<A: Actor> MailboxSender<A> for Sender<Signal<A>> {
    ...
    fn try_send(&self, signal: Signal<A>) -> Result<(), MailboxTrySendError<A>> {
        self.try_send(signal).map_err(|err| match err {
            async_channel::TrySendError::Full(signal) => MailboxTrySendError::Full(signal),
            async_channel::TrySendError::Closed(signal) => MailboxTrySendError::Closed(signal),
        })
    }
    ...
    fn send_timeout(
        ...
        async move {
            match tokio::time::timeout(timeout, self.send(signal)).await {
                Ok(Ok(())) => Ok(()),
    ...
    fn weak_count(&self) -> usize {
        unimplemented!("async_channel doesn't support weak_count")
    }
}
...
impl<A: Actor> WeakMailboxSender<A> for WeakSender<Signal<A>> {
    ...
    fn strong_count(&self) -> usize {
        unimplemented!("async_channel doesn't support strong_count")
    }
...
impl<A: Actor> MailboxReceiver<A> for Receiver<Signal<A>> {
    fn recv(&mut self) -> BoxFuture<'_, Option<Signal<A>>> {
        async move { (Receiver::recv(self).await).ok() }.boxed()
    }

    fn try_recv(&mut self) -> Result<Signal<A>, MailboxTryRecvError> {
        Receiver::try_recv(self).map_err(|err| match err {
            async_channel::TryRecvError::Empty => MailboxTryRecvError::Empty,
            async_channel::TryRecvError::Closed => MailboxTryRecvError::Closed,
        })
    }

    fn blocking_recv(&mut self) -> Option<Signal<A>> {
        Receiver::recv_blocking(self).ok()
    }
    ...
    fn sender_weak_count(&self) -> usize {
        unimplemented!("async_channel doesn't support sender_weak_count")
    }
}
