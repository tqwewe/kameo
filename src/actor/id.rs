...
///
/// `ActorId` wraps a locally sequential `sequence_id` assigned to each actor as it is
/// spawned.
...
impl ActorId {
    /// Creates a new `ActorId` with the given `sequence_id`.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// The resulting vector contains the `sequence_id` in little-endian bytes.
    ///
    /// # Returns
    ///
