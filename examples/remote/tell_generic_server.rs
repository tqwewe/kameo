...
impl Message<StoreItem> for StorageActor {
    ...
    async fn handle(
        ...
        println!("📦 Stored item '{}' (total items: {}, message #{})", 
                 msg.item, self.items.len(), self.message_count);
    }
}
...
impl Message<ClearStorage> for StorageActor {
    ...
    async fn handle(
        ...
        self.items.clear();
        println!("🗑️  Cleared {} items from storage (message #{})", count, self.message_count);
    }
}
...
impl Message<PrintItems> for StorageActor {
    ...
    async fn handle(
        ...
        self.message_count += 1;
        println!("📋 Storage contents ({} items, message #{}):", self.items.len(), self.message_count);
...
impl StorageActor {
    async fn handle_store_item(&mut self, msg: &rkyv::Archived<StoreItem>) {
        ...
        println!("📦 Stored item '{}' (total items: {}, message #{})", 
                 item, self.items.len(), self.message_count);
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    transport.register_actor("storage".to_string(), storage_id).await?;
    ...
    if let Some(handle) = transport.handle() {
        let _peer = handle.add_peer(&kameo_remote::PeerId::new("kameo_node_9321")).await;
