...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(handle) = transport.handle() {
        let server_peer_id =
            kameo_remote::KeyPair::new_for_testing("ask_candle_server_test_key").peer_id();
    ...
    let message_size =
        std::mem::size_of::<FuturesOHLCVCandle>() + std::mem::size_of::<CandleAnalysis>();
    ...
    println!(
        "   Latency (avg/min/max): {:.0}/{:.0}/{:.0} μs",
        avg_latency_us, min_latency_us, max_latency_us
    );
    println!(
        "   Total data exchanged: {:.2} MB",
