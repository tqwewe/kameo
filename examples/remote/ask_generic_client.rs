...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(handle) = transport.handle() {
        let peer = handle.add_peer(&kameo_remote::PeerId::new("kameo_node_9340")).await;
    ...
    let calc_ref = match DistributedActorRef::<CalculatorActor>::lookup("calculator", transport).await? {
        Some(ref_) => {
    ...
    println!("✅ Got response: value={:?}, operation_count={}", result.value, result.operation_count);
