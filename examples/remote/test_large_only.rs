...
impl TestActor {
    async fn handle_large(&mut self, msg: &rkyv::Archived<LargeTestMessage>) {
        ...
        println!("✅ RECEIVED: {}MB message (actual: {} bytes, count: {})", 
...
async fn run_server() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    transport.register_actor("test_large".to_string(), actor_id).await?;
    ...
    println!("🔍 Server LargeTestMessage type hash: {:08x}", <LargeTestMessage as HasTypeHash>::TYPE_HASH.as_u32());
    println!("✅ TestActor registered with ID {:?}", actor_id);
    
    // Add client as peer
    if let Some(handle) = transport.handle() {
        let _peer = handle.add_peer(&kameo_remote::PeerId::new("kameo_node_9401")).await;
...
async fn run_client() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Connect to server
    if let Some(handle) = transport.handle() {
        let peer = handle.add_peer(&kameo_remote::PeerId::new("kameo_node_9400")).await;
    ...
    use kameo::remote::type_hash::HasTypeHash;
    println!("🔍 Client LargeTestMessage type hash: {:08x}", <LargeTestMessage as HasTypeHash>::TYPE_HASH.as_u32());
