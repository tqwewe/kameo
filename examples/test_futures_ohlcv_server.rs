...
mod app {
    ...
    #[derive(
        kameo::RemoteMessage, Clone, Debug, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
    )]
    ...
    impl TradingActor {
        async fn handle_candle_message(&mut self, msg: &rkyv::Archived<CandleMessage>) {
            ...
            if self.candles_received == 1 {
                ...
                let mut deserializer = rkyv::rancor::Strategy::<
                    rkyv::rancor::Failure,
                    rkyv::rancor::Panic,
                >::wrap(&mut failure);
    ...
    pub async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        transport
            .register_actor("trading_actor".to_string(), actor_id)
            .await?;
        ...
        if let Some(handle) = transport.handle() {
            let client_peer_id =
                kameo_remote::KeyPair::new_for_testing("test_futures_client").peer_id();
