...
impl Message<TestDataMessage> for TestActor {
    ...
    async fn handle(&mut self, msg: TestDataMessage, _ctx: &mut Context<Self, Self::Reply>) -> Self::Reply {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    transport.register_distributed_actor("test_actor".to_string(), &actor_ref).await?;
